...
openssl req -x509 -nodes -days 365 -newkey rsa:2048 -out $CERT_PATH -keyout $CERTKEY_PATH -subj "/C=ES/ST=Andalucia/L=Malaga/O=42/OU=42/CN=$DOM_NAME/UID=$LOGIN"

# replace hostname, crt and key paths in the nginx.conf file
sed -i "s/dom_name/$DOM_NAME/" /etc/nginx/nginx.conf
